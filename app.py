...
def router():
    ...
    for item in business.json():
        if groups and data["groups"][0].get("name", "").upper() == item["name"].upper():
            business_code = item["business_code"]
            break
        else:
            labels[item["business_code"]] = requests.get(
                f"https://tim-loonie-94123.herokuapp.com/business/{item['id']}/labels")

            for label in labels[item["business_code"]].json():
                beetrack_dispatch_response = label.get(
                    "beetrack_dispatch_response")
                if beetrack_dispatch_response and "identifier" in beetrack_dispatch_response.get("response", {}):
                    if beetrack_dispatch_response["response"]["identifier"].lower() == data["identifier"].lower():
                        business_code = item["business_code"]
                        break
